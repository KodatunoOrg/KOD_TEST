...
int SmpContourLine(BODYList *BodyList,OBJECTList *ObjList, int PickCount, double Prop[])
{
	...
	for(int i=0;i<step;i++){
		...
		t = S->CalcIntersecPtsPlaneSearch(pt,nvec,feed,5,RUNGE_KUTTA);		// NURBS曲面と平面との交点群を交線追跡法で求める
...
int SmpIntersectSurfs(BODYList *BodyList,OBJECTList *ObjList, int PickCount, double Prop[])
{
	...
	VCoord Rt;						// NURBS曲面R(w,t)における解
	VCoord St;						// NURBS曲面S(u,v)における解
	...
	NURBSS *S1 = body1->m_vTrmS[obj1->Num]->m_pts;	// BODY1からNURBS曲面を取り出す
	NURBSS *S2 = body2->m_vTrmS[obj2->Num]->m_pts;	// BODY2からNURBS曲面を取り出す

	boost::tie(Rt, St) = S1->CalcIntersecPtsNurbsSSearch(S2,10,feed);	// NURBS曲面同士の交線算出
...
int SmpMeanCurvature(BODYList *BodyList,OBJECTList *ObjList, int PickCount, double Prop[])
{
	...
	for(int i=0;i<=udiv;i++){
		double u = S->m_U[0]+(double)i*(S->m_U[1] - S->m_U[0])/(udiv+1);
		for(int j=0;j<=vdiv;j++){
			double v = S->m_V[0]+(double)j*(S->m_V[1] - S->m_V[0])/(vdiv+1);
